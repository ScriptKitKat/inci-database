...
class WikidataNameLookup:
    ...
    def lookup(self, normalized_name: str) -> list[IngredientEvidence]:
        ...
        resp = self._client.get(WIKIDATA_SPARQL, params={"query": query, "format": "json"})
        if resp.status_code != 200:
...
def incidecoder_lookup(client: httpx.Client | None = None) -> ExactIngredientPageLookup:
...
def specialchem_lookup(client: httpx.Client | None = None) -> ExactIngredientPageLookup:
    return ExactIngredientPageLookup(
        "specialchem",
